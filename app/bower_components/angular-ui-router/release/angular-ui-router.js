...
var PathUtils = (function () {
     ...
     *
     * On each [[PathNode]], creates ViewConfig objects from the app: property of the node's state
     */
...
var Transition = (function () {
     ...
     *
     * Each state can define one or more app (template/controller), which are encapsulated as `ViewConfig` objects.
     * This method fetches the `ViewConfigs` for a given path in the Transition (e.g., "to" or "entering").
     *
     * @param pathname the name of the path to fetch app for:
...
var StateBuilder = (function () {
    function StateBuilder(matcher, urlMatcherFactory) {
        ...
        this.builders = {
            ...
            // Each framework-specific ui-router implementation should define its own `app` builder
            // e.g., src/ng1/statebuilders/app.ts
            views: [],
 ...
 * with view configs (from the state declaration objects: [[StateDeclaration.app]]).
 *
 * - After a successful Transition, the app from the newly entered states are activated via [[activateViewConfig]].
 *   The app from exited states are deactivated via [[deactivateViewConfig]].
...
var ViewService = (function () {
    ...
    ViewService.prototype.sync = function () {
        ...
        // The depth is the nesting depth of ui-app (based on FQN; times 10,000)
        ...
        var configureUIView = function (_a) {
            ...
            // If a parent ui-view is reconfigured, it could destroy child ui-app.
            // Before configuring a child ui-view, make sure it's still in the active uiViews array.
            if (_this._uiViews.indexOf(uiView) !== -1)
                uiView.configUpdated(viewConfig);
        };
        // Sort app by FQN and state depth. Process uiviews nearest the root first.
    ...
    /**
     * Returns the list of app currently available on the page, by fully-qualified name.
     *
    ...
    /**
     * Returns the list of app on the page containing loaded content.
     *
     * @return {Array} Returns an array of fully-qualified view names.
     */
    ...
    /**
     * Normalizes a view's name from a state.app configuration block.
     *
     * This should be used by a framework implementation to calculate the values for
     * [[_ViewDeclaration.$uiViewName]] and [[_ViewDeclaration.$uiViewContextAnchor]].
     *
     * @param context the context object (state declaration) that the view belongs to
     * @param rawViewName the name of the view, as declared in the [[StateDeclaration.app]]
     *
...
/**
 * A [[TransitionHookFn]] which waits for the app to load
 *
 * Registered using `transitionService.onStart({}, loadEnteringViews);`
 *
 * Allows the app to do async work in [[ViewConfig.load]] before the transition continues.
...
/**
 * A [[TransitionHookFn]] which activates the new app when a transition is successful.
 *
 * Registered using `transitionService.onSuccess({}, activateViews);`
 *
 * After a transition is complete, this hook deactivates the old app from the previous state,
 * and activates the new app from the destination state.
 *
 * See [[ViewService]]
 */
...
/**
 * This is a [[StateBuilder.builder]] function for angular1 `app`.
 *
 * When the [[StateBuilder]] builds a [[StateObject]] object from a raw [[StateDeclaration]], this builder
 * handles the `app` property with logic specific to @uirouter/angularjs (ng1).
 *
 * If no `app: {}` property exists on the [[StateDeclaration]], then it creates the `app` object
...
function ng1ViewsBuilder(state) {
    ...
    // Do not allow a state to have both state-level props and also a `app: {}` property.
    // A state without a `app: {}` property can declare properties for the `$default` view as properties of the state.
    // However, the `$default` approach should not be mixed with a separate `app: ` block.
    if (isDefined(state.views) && hasAnyKey(allViewKeys, state)) {
        throw new Error("State '" + state.name + "' has a 'app' object. " +
            "It cannot also have \"view properties\" at the state level.  " +
            "Move the following properties into a view (in the 'app' object): " +
    ...
    forEach(viewsObject, function (config, name) {
        // Account for app: { "": { template... } }
        name = name || "$default";
        // Account for app: { header: "headerComponent" }
        if (isString(config))
...
var StateProvider = (function () {
     ...
     * - **app** `{object}` - returns a app object where each key is an absolute view
     *   name (i.e. "viewName@stateName") and each value is the config object
     *   (template, controller) for the view. Even when you don't use the app object
     ...
     *
     * #### Example:
     * Override the internal 'app' builder with a function that takes the state
     * definition, and a reference to the internal function being overridden:
     * ```js
     * $stateProvider.decorator('app', function (state, parent) {
     *   let result = {},
     *       app = parent(state);
     *
     *   angular.forEach(app, function (config, name) {
     ...
     *
     * ```js
     * // Auto-populates list and item app with /partials/home/contact/list.html,
...
function $uiRouter($locationProvider) {
    ...
    // Apply ng1 specific StateBuilder code for `app`, `resolve`, and `onExit/Retain/Enter` properties
 ...
 *   The name should be unique amongst the other app in the same state.
 *   You can have app of the same name that live in different states.
 *   The ui-view can be targeted in a View using the name ([[Ng1StateDeclaration.app]]).
 *
 ...
 * [[Ng1StateDeclaration.app]] config property, by name, in this case an empty name:
 *
 * ```js
 * $stateProvider.state("home", {
 *   app: {
 ...
 * ```
 *
 * But typically you'll only use the app property if you name your view or have more than one view
 ...
 * ```
 *
 * Really though, you'll use app to set up multiple app:
 *
 * ```html
 * <div ui-view></div>
